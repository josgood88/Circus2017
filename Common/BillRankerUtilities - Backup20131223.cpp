...
public:
   Workers(int numberOfThreads, const std::string& textToProcess, const std::map<std::string, std::pair<std::regex, int>> rankingTerms, bool _showDetails) {
      ...
      std::for_each(rankingTerms.cbegin(),rankingTerms.cend(), [&] (const std::pair<std::string, std::pair<std::regex, int>> term) { 
         ...
         m_threads.create_thread(boost::bind(&Workers::Worker, this, w));
      });
   }
   ~Workers() {
      m_io.stop();
...
private:
   void Spawn(const WorkerArgs w) {
   }

   void Worker(const WorkerArgs w) {
      boost::mutex::scoped_lock lock(thread_control);
      std::cout <<  w.term << " started." << std::endl;
   ...
   boost::mutex mut_score;
   boost::mutex thread_control;
   bool showDetails;
};
